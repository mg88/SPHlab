...
% 
            ...
            qrho_ji_diss(obj.active_k_pij,:) = alpha_mass* ... % i->j
                obj.mj(obj.Ii).*...
            ...
            % compute fluxes
            blub = 0*qrho_ij_diss;
            blub(obj.active_k_pij,:) = alpha_mass* ... % j->i
            ...
            % spread flux onto nodes
            bla = (obj.AedgesXj*blub)./obj.mj;
                          
            obj.drhoj(obj.Icomp,:) = obj.drhoj(obj.Icomp,:) ...
                                        + obj.drhoj_diss(obj.Icomp,:);

            if any(norm(bla-obj.drhoj_diss)/norm(bla)>1e-10)
                keyboard
            end
            ...
            if ~obj.isothermal
                ...
                obj.dej(obj.Icomp,:) = obj.dej(obj.Icomp,:)+ de;
                %--------------
...
%                     % thermal conductivity
                ...
                % spread flux onto nodes
                obj.dej_diss = ( ... (obj.AedgesXj * qe_ij_sym)./obj.mj +...
...
%         %            obj.vj(kb_stop) = 0;
        ...
        %%                       
        function mirrorParticles (obj) % for boundary conditions 
...
%                     end
                    ...
                    %change order (for better readablity)
                    d=flip(d);
                ...
                if ~isempty(Imirror)
                    ...
                    obj.vj (obj.N+(1:NN),:) = ...    -...
                              obj.vj(Imirror,:).*(ones(NN,1)*v_factor);
                          % ----
                    if flag_compute_ghost
                        obj.vj_half (obj.N+(1:NN),:) = ...    -...
                                  obj.vj_half(Imirror,:).*(ones(NN,1)*v_factor);
                        obj.ej_half (obj.N+(1:NN),:) = ...    -...
                                   p_factor.*obj.ej_half(Imirror,:);
                        obj.c0j (obj.N+(1:NN),:)   = obj.c0j(Imirror,:);
                        obj.rho0j (obj.N+(1:NN),:) = obj.rho0j(Imirror,:);
                        obj.rhoj_half (obj.N+(1:NN),:) = obj.rhoj_half(Imirror,:);
                    end
                           %-----
                    obj.ej (obj.N+(1:NN),:) = ...
                              p_factor.*obj.ej(Imirror);                          
                    obj.pj (obj.N+(1:NN),:) = ...                            
                              p_factor.*obj.pj(Imirror); 
